$(document).ready(function () {
  ...
  $(document).on("click", ".btn-gift", function () {
    ...
    if (!giftShowing) {
      $(".btn-gift").html(
        giftButtonText + ' <i class="fa fa-chevron-up" aria-hidden="true"></i>'
      );
  ...
  $(".gifting").on("change", "input", function () {
    document.cookie =
      "buycraft_gifting=" + $(".gifting").find("input").serialize();
  });
  ...
  $.ajax({
    ...
    success: function (json) {
      if (json.type == "error" && json.success_url == "redirect") {
        ...
        if (
          typeof reRenderWidgets != "undefined" &&
          json.success_url == "rerender"
        ) {
          reRenderWidgets();
        }
        clearWaitingOverlay();
    ...
    error: function (data) {
      notification.show(
        "danger",
        "We could not send you to Tebex Checkout - please refresh the page and try again."
      );
      button.button("reset");
    },
  });
...
notification = new (function () {
  this.show = function (type, message) {
    ...
    $(".notification")
      .empty()
      .append('<div class="alert alert-' + type + ' alert-dismissable"></div>');
    $(".notification .alert")
      .append(
...
if (document.getElementById("copyWebstoreIp") !== null) {
  document
    .getElementById("copyWebstoreIp")
    .addEventListener("click", function () {
      copyToClipboard(document.getElementById("webstoreIp"));
      document.getElementById("copyWebstoreIp").innerHTML =
        "Copied <i class='fas fa-check'></i></i>";
    });
}
